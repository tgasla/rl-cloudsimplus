...
def main():
    ...
    parser.add_argument("--id-start-from", type=int, default=0)
    parser.add_argument("--lambda-poisson", type=int, default=1)
    parser.add_argument("--mu-gaussian", type=int, default=1)
    parser.add_argument("--sigma-gaussian", type=int, default=1)
    parser.add_argument("--jobs", type=int)
    parser.add_argument("--mi-multiplier", type=int, default=1000)
...
def generate_job_trace(args):
    ...
    if args.distribution == "poisson":
        arrival_times = args.start_from + np.cumsum(
            np.random.poisson(args.lambda_poisson, args.jobs)
        )
    elif args.distribution == "gaussian":
        arrival_times = args.start_from + np.abs(
            np.cumsum(
                np.random.normal(args.mu_gaussian, args.sigma_gaussian, args.jobs)
            )
        )
    else:
        raise ValueError(
            "Unsupported distribution. Choose either 'poisson' or 'gaussian'."
        )
